...
(function (global, factory) {
      ...
      setChildren() {
          ...
          for (let i = 0; i < this.children.length; i++) {
              ...
              if (Array.isArray(result)) {
                  ...
                  this.VNodeChildren[i] = initVNodeWithList(result, this);
                  addFragmentEle(this.fragment, this.VNodeChildren[i], pivot);
                  let fn = () => {
                      removeFragmentEle(this.VNodeChildren[i]);
                      let newVnodeList = child();
                      if (Array.isArray(newVnodeList)) {
                          this.VNodeChildren[i] = initVNodeWithList(newVnodeList, this);
                          addFragmentEle(this.getParentMountedEle(), this.VNodeChildren[i], pivot);
                      }
                  };
                  ...
                  let fn = () => {
                      let newNode = child();
                      this.VNodeChildren[i] = replaceElementChild(this.getParentMountedEle(), initVNode(newNode, this), this.VNodeChildren[i]);
                  };
                  depProps.forEach(propValue => propValue.setDep(new Watcher(fn)));
              }
          }
  ...
  */
  function replaceElementChild(ele, newNode, oldNode) {
      ...
      let newEXNode;
      let oldEXNode;
      if (newNode instanceof ComponentVNode)
          newEXNode = newNode.getComponent().getEFVNode();
      else
          newEXNode = newNode;
      if (oldNode instanceof ComponentVNode)
          oldEXNode = oldNode.getComponent().getEFVNode();
      else
          oldEXNode = oldNode;
  ...
  // 替换为FragmentVNode时
  function replaceToFragmentVNode(ele, newNode, oldNode) {
  ...
  // 被替换为FragmentVNode时
  function replacedByFragmentVNode(ele, newNode, oldNode) {
  ...
  // 移除Fragment节点
  function removeFragmentNode(node) {
