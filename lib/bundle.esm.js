 ...
 * 监控数据响应变化处
*/
class DataProxy extends EventEmitter {
    constructor(data) {
        super();
        this.valueMap = new WeakMap();
...
class Component$1 {
    ...
    renderRoot() {
        return this.render(createVNode$1);
    }
}
        ...
        if (this.props) {
            for (let prop in this.props) {
                if (typeof this.props[prop] === 'function') {
                    let event = ElementVNode.nativeEvents[prop];
                    if (event)
                        this.ele.addEventListener(event, this.props[prop].bind(this.ele));
                    continue;
                }
                if (this.props[prop] === 'style') ;
            }
        }
        // 处理标签子节点
...
const createVNode = createVNode$1;

export { Component, createVNode, mount };
